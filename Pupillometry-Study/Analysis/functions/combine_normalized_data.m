...
function [anech_pupil_uninter,anech_pupil_inter,reverb_pupil_uninter,reverb_pupil_inter ] = combine_normalized_data(included_subj,fs,N_EnvTrials,task_analysis,N_samples_task_anech,N_samples_task_reverb)

for i = 1:length(included_subj)
    new_field_name = append("P",included_subj(i));

    %%%%%%%%%%%%%%%% TASK EVOKED
    [anech_uninter_data_norm_task,anech_inter_data_norm_task, anech_baseline_period(i,:)] = normalize_pupil_data(fs,N_EnvTrials,task_analysis.(new_field_name).anech_pupil_uninter,task_analysis.(new_field_name).anech_pupil_inter,N_samples_task_anech);
    [reverb_uninter_data_norm_task,reverb_inter_data_norm_task, reverb_baseline_period(i,:)] = normalize_pupil_data(fs,N_EnvTrials,task_analysis.(new_field_name).reverb_pupil_uninter,task_analysis.(new_field_name).reverb_pupil_inter,N_samples_task_reverb);
