...
function raw_trial_data = interpolate_data(subj_ID,raw_trial_data,time_idx,data_col,time_stamps,blink_time_idx)
...
for iInt = 1:length(time_idx)
    ...
    elseif strcmp(subj_ID,'001')
        % if iInt == 43 
        %     data_to_interp(1:84,:) = 0;
        %     interp_blink_time_idx = [];
        if iInt == 129
            data_to_interp(1:63,:) = 0;
            interp_blink_time_idx = [];
        end
    % elseif strcmp(subj_ID,'017') %trial begins with eye blink
    %     if iInt == 96
    %         data_to_interp(1:273,:) = 0;
    %         interp_blink_time_idx = [];
    %     end
    end
    ...
    % if iInt == 132 %this is because there is only 1 blink in entire trial; P005
    ...
    % if iInt == 1
    %     new_interp_blink_time_first(1:2,:) = [ ]; %only for first trial
