...
% https://github.com/haiderriazkhan/microsaccade/tree/master

% function microsaccades = micsaccdeg(EyeDeg, SAMPLING)
function [vel,microsaccades] = micsaccdeg(EyeDeg, SAMPLING)
...
% vel = vecvel(EyeDeg,SAMPLING,2);
% v(:,1) = sqrt(vel(:,1).^2 + vel(:,2).^2);
...
while(i<=N)
    ...
    % if vel(i) >=6 %speed crosses 6 deg/s
    if vel(i) >=8 %speed crosses 8 deg/s
        % while(vel(i+j) >= 6)
        while(vel(i+j) >= 8)
